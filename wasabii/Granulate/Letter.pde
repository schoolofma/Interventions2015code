...
class Letter {
  ...
  Letter(int shape) {
    ...
    if (shape==5) {
      ...
      creator.setEdge(10); // edge length of the polyhedron
  ...
  void updatePshape() {
    ...
    for (int i=0; i<count; i++) {  //to go through each single vertex
      ...
      pipi.setTextureUV(i, noise(v.x*0.1, v.z*0.1), noise(v.y*0.01, randomizer)); //noise is a function to generate organic movements
    }
  ...
  void dodistort() {
    ...
    if (currMod==0) {//2
      ...
      HEM_Extrude extrude= new HEM_Extrude().setDistance(0.001).setChamfer(3);
      mesh.modify(extrude);
      updatePshape();
    }
    if (currMod==1) {//U
    ...
    if (currMod==3) {//m
      ...
      HEM_Bend bendy= new HEM_Bend().setAngleFactor(0.5).setGroundPlane(P).setBendAxis(L);
      mesh.modify(bendy);
      updatePshape();
    }
    if (currMod==4) {//o
      HEM_Twist twisty= new HEM_Twist().setTwistAxis(new WB_Line(0, 0, 0, 1, 1, 1)).setAngleFactor(radians(0.1));
      mesh.modify(twisty);
      updatePshape();
    }
    if (currMod==5) {//=
      HEM_PunchHoles holes= new HEM_PunchHoles().setWidth(50);
