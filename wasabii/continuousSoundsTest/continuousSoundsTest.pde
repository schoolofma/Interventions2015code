...
void setup() {
  ...
  instruments = new ArrayList<Synth>();

  render=new WB_Render(this);
  minim = new Minim( this );
  out   = minim.getLineOut();
  //creates 3 instruments
  instruments.add(new Synth( Waves.TRIANGLE, 10f, 0.5));//0
  instruments.add(new Synth( Waves.TRIANGLE, 1320f, 0.3));//1
  instruments.add(new Synth( Waves.TRIANGLE, 775f, 0.5));//2
  instruments.add(new Synth(Waves.SINE, 440f, 0.5));//3 
  instruments.add(new Synth(Waves.SAW, 440f, 0.5));//4
  instruments.add(new Synth(Waves.TRIANGLE, 440f, 0.5));//5
  instruments.add(new Synth(Waves.SQUARE, 440f, 0.5));//6
  instruments.add(new Synth(Waves.QUARTERPULSE, 440f, 0.5));//7
}
...
void draw() {
  ...
  for (int i=shapes.size ()-1; i>=0; i--) {
    ...
    if (l.y < 0) {
      shapes.remove(i);
      //if no cubes left, turn cube sound off. 
      println("remove", i);
    }
  }
  if (frameCount% 60==0) {
...
void mouseMoved()
  {
    ...
    instruments.get(3).fm.setFrequency( modulateFrequency );
    instruments.get(3).fm.setAmplitude( modulateAmount );
  }
