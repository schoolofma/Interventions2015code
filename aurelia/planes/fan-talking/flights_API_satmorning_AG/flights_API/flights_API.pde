...
//THIS METHOD COULD USE SOME REFACTORING (currentFlightPosition should be in Flight class for example).
void moveToNextFlightPosition(){
  println("flights.size(): " + flights.size());
  ...
  for(int j=0; j<cols; j++){
    for(int i=0; i<rows; i++){
      ...
      if (latitudeInPixels > thisFan.row  && latitudeInPixels < thisFan.row+thisFan.side/2 && 
        longitudeInPixels > thisFan.col-thisFan.side/2 && longitudeInPixels < thisFan.col+thisFan.side/2) { 
