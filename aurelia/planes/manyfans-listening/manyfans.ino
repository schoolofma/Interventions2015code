...
void loop()
{
  if (Serial.available()) {
     //for one fan
      ...
      analogWrite(pmwpin, processingVal.parse);
    ...
    if (Serial.find("a")) { //everytime there is an "a" it's going to write the next values
      ...
      analogWrite(pins[i], processingVal[i]; // send values to motor : By changing values from 0 to 255 you can control motor speed
                  //  analogWrite(pmwpin, motorValue); // send values to motor : By changing values from 0 to 255 you can control motor speed
